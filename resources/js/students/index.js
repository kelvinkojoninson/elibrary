...
var InitStudents = function () {
    ...
    const FETCH_API_ENPOINT = APP_URL + '/api/students'; // API endpoint for fetching roles
    const CREATE_API_ENPOINT = APP_URL + '/api/students'; // API endpoint for creating
    const UPDATE_API_ENPOINT = APP_URL + '/api/students/update'; // API endpoint for updating
    const DELETE_API_ENDPOINT = APP_URL + '/api/students/delete'; // API endpoint for deleting
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        if (permissions.modUpdate == 1) {
                            actions += `<a href="${APP_URL}/student/${data.id}" class="text-info me-3">
                                            View
                                        </a>`;
                        }
    ...
    const handleUpdateRows = () => {
        ...
        buttons.forEach(button => {
            button.addEventListener('click', function (e) {
                ...
                $("#update-student-id").val(data.studentId);
                $("#update-first-name").val(data.firstName);
                $("#update-last-name").val(data.lastName);
                $("#update-other-name").val(data.otherNames);
                $("#update-class").val(data.classId).trigger("change");
