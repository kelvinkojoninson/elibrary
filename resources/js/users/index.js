"use strict";
/**
InitUsers is a module that handles the initialization and functionality of a user management CRUD.
...
var InitUsers = function () {
    ...
    const FETCH_API_ENPOINT = APP_URL + '/api/users'; // API endpoint for fetching
    const CREATE_API_ENPOINT = APP_URL + '/api/users'; // API endpoint for creating
    const UPDATE_API_ENPOINT = APP_URL + '/api/users/update'; // API endpoint for updating
    const DELETE_API_ENDPOINT = APP_URL + '/api/users/delete'; // API endpoint for deleting
        ...
        datatable = $(table).DataTable({
            ...
            columns: [{
                ...
                },
                {
                    ...
                    <div class="symbol symbol-70px me-3">
                        <div class="symbol-label">
                            <img src="${data.picture ? data.picture : '/assets/images/user.png' }" alt="${data.name}" class="img-card w-100 h-100" />
                        </div>
                    </div>`
                    },
                },
                ...
                },
                {
                    data: "email",
                    render: function (data, type, full, meta) {
                        return `<a class="text-gray-700" href="mailto:${data}">${data}</a>`
                    },
                },
                {
                    data: "phone"
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        return `<div class="badge badge-${data.roleBadge}">${data.roleDesc}</div>`;
                    },
                },
                {
                    data: "lastLogin"
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        return `<div class="d-flex flex-column">
                            <span class="text-gray-800 mb-1">${data.dateCreated}</span>
                            <span class="text-gray-700 fs-7">${data.createdBy}</span>
                        </div>`;
                    },
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        return `<div class="d-flex flex-column">
                            <span class="text-gray-800 mb-1">${data.dateUpdated}</span>
                            <span class="text-gray-800 fs-7">${data.updatedBy}</span>
                        </div>`;
                    },
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        let html = ``;
                        if (data == 'ACTIVE')
                            html += `<div class="bg-success">Active</div>`;
                        else if (data == 'INACTIVE')
                            html += `<div class="bg-warning">Inactive</div>`;
                        else if (data == 'SUSPENDED')
                            html += `<div class="bg-danger text-white">Suspended</div>`;
    ...
    const handleUpdateRows = () => {
        ...
        buttons.forEach(button => {
            button.addEventListener('click', function (e) {
                ...
                if (Boolean(data.picture)) document.getElementById('update-img-holder').style.backgroundImage = "url(" + data.picture + ")";
                document.getElementById('update-code').value = data.id;
                ...
                $('#update-role').val(data.roleId).trigger('change');
            })
        });
    }
